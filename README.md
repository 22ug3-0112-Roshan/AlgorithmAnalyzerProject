# AlgorithmAnalyzerProject
CIT300 Graded Practical Assignment 3 (Week 14): Algorithm Analyzer – Measuring Performance &amp; Complexity
<div align="center">
---

## 🚀 **Project Overview**
This project evaluates **execution time** of different algorithms using Java.  
Each team member implemented one algorithm, executed it on input sizes:

- **100**
- **500**
- **1000**

Time was measured using `System.nanoTime()` for accuracy.

The aim is to understand:
- Time complexity  
- Practical runtime behavior  
- Differences between searching & sorting algorithms  
- How input size impacts performance  

---

## 👥 **Team Role Distribution**

| Member | Task | Algorithm Type |
|--------|------|----------------|
| **Member 1** = W.M.D.D.WANNINAYAKA ( 22UG3-0264) | Linear Search | Searching |
| **Member 2** = W.M.A.N.MADUMALI ( 22UG3-0440)| Binary Search (after sorting) | Searching |
| **Member 3** = D.M.S.U.DISSANAYAKA (22UG3-0342) | Bubble Sort | Sorting |
| **Member 4** = R.R.R.L.KULARATHNA (22UG3-0112) | Quick Sort | Sorting |

---

## 📂 **Project Structure**

```txt
AlgorithmAnalyzerProject/
│
├── Member1/
│   └── LinearSearchTest.java
│
├── Member2/
│   └── BinarySearchTest.java
│
├── Member3/
│   └── BubbleSortTest.java
│
└── Member4/
    └── QuickSortTest.java
